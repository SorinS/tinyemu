...
var mustBeWiredSingle = []byte{
	...
	0x84, 0x85, 0x86, 0x87,
	0x88, 0x89, 0x8A, 0x8B,
	// 0x8D LEA — wired but needs memory operand; sweep encodes reg-form
	0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97,
	0x98, 0x99,
	0xA4, 0xA5, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC, 0xAD, 0xAE, 0xAF,
	0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, 0xB6, 0xB7,
	0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF,
	0xC1, 0xC3, 0xC6, 0xC7, 0xC9,
	// 0xCB RETF wired (operand-size 8 only); 0xCD INT wired (needs IDT)
	0xD0, 0xD1, 0xD2, 0xD3,
	0xE8, 0xE9, 0xEB,
...
var mustBeWiredTwoByte = []byte{
	...
	0xA2, 0xA3, 0xA4, 0xA5, 0xAB, 0xAC, 0xAD, 0xAF,
	// 0xAE Group 15 (FXSAVE/etc) needs memory operand — wired, sweep encoding wrong
	0xB0, 0xB1, 0xB3, 0xB6, 0xB7, 0xBA, 0xBB, 0xBC, 0xBD, 0xBE, 0xBF,
	0xC0, 0xC1, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD, 0xCE, 0xCF, // XADD, BSWAP
	// Critical SSE-FP we added today (scalar forms require F2/F3 prefix;
	// the sweep encodes no-prefix which is the packed-int conversion
	// form we don't yet wire — kept here only for forms that work
	// without a prefix).
	0x51, 0x58, 0x59, 0x5C, 0x5D, 0x5E, 0x5F, 0xC2, 0xC6,
	// Common SSE2 moves/arith
	0x10, 0x11, 0x28, 0x29, 0x6E, 0x6F, 0x7E, 0x7F,
	0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68,
	// 0x6C/0x6D — PUNPCK*QDQ need 66 prefix; sweep tests no-prefix
	0x70, 0x74, 0x75, 0x76,
	0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD7, 0xDB, 0xDF,
	// 0xD6 MOVQ-store needs memory dest; 0xE7 MOVNTDQ needs memory dest
	0xEB, 0xEF, 0xF4, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE,
}
...
func TestOpcodeSweep(t *testing.T) {
	...
	// Optional stable Markdown inventory for review / regression tracking
	// (advisor report #3). Set TINYEMU_X64_SWEEP_INVENTORY=<path> to emit
	// it; rows are already in opcode order so the file diffs cleanly when
	// coverage changes. The path must be absolute (go test runs in the
	// package directory). Regenerate from the repo root with:
	//   TINYEMU_X64_SWEEP_INVENTORY="$PWD/docs/opcode-coverage.md" \
	//     go test ./cpu/x86_64 -run TestOpcodeSweep
	if path := os.Getenv("TINYEMU_X64_SWEEP_INVENTORY"); path != "" {
		var b strings.Builder
		fmt.Fprintf(&b, "# x86_64 opcode coverage\n\n")
		fmt.Fprintf(&b, "Generated by `TestOpcodeSweep` (cpu/x86_64/opspec_test.go). Do not edit by hand.\n\n")
		fmt.Fprintf(&b, "Status: `OK` wired · `UNIMPL` decoder gap (not yet written) · `OTHER` dispatched but errored — check encoding · `PREFIX` prefix byte.\n\n")
		fmt.Fprintf(&b, "**Coverage: PREFIX=%d · OK=%d · UNIMPL=%d · OTHER=%d · total %d**\n\n",
			counts[resultPrefix], counts[resultOK], counts[resultUnimpl], counts[resultOther], len(rows))
		section := func(title string, twoByte bool) {
			fmt.Fprintf(&b, "## %s\n\n| Opcode | Status |\n|---|---|\n", title)
			for _, r := range rows {
				if (r.op > 0xFF) != twoByte {
					continue
				}
				fmt.Fprintf(&b, "| %02X | %s |\n", r.op&0xFF, r.res)
			}
		...
		section("Single-byte opcodes", false)
		section("Two-byte opcodes (0F XX)", true)
		if err := os.WriteFile(path, []byte(b.String()), 0o644); err != nil {
			t.Fatalf("write opcode inventory %s: %v", path, err)
		}
		t.Logf("wrote opcode inventory to %s", path)
	}
