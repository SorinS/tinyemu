...
func TestPMDebug(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		fmt.Printf("Step %d: EIP=%08X LIP=%08X byte=%02X EAX=%08X PE=%v CS=%04X\n",
			i, c.GetEIP(), lip, b, c.GetReg32(EAX), c.IsProtectedMode(), c.GetSeg(CS))
