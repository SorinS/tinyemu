...
func TestPMDebug3(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		fmt.Printf("Step %d: EIP=%08X LIP=%08X byte=%02X PE=%v CS=%04X\n",
			i, c.GetEIP(), c.GetLIP(), func() uint8 { b, _ := c.memMap.Read8(uint64(c.GetLIP())); return b }(), c.IsProtectedMode(), c.GetSeg(CS))
