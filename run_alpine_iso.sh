...
#       built-in.
#   autodetect_serial=no               — Alpine's /init calls
#       `setup_inittab_console` post-apk, which calls `detect_serial_consoles`
#       which runs `awk` on /proc/tty/driver/serial. That awk currently
#       hangs (same project_awk_hang.md class). Setting this kopt makes
#       detect_serial_consoles return immediately. We still get a console
#       on ttyS0 because of the `console=ttyS0,115200` kopt.
#
...
exec bin/temu.darwin-arm64.bin \
    ...
    -net-user \
    -append "console=ttyS0,115200 noapic nolapic acpi=off pci=noacpi nosmp nokaslr tsc=reliable libata.force=disable ide=disable alpine_dev=vda:iso9660 usbdelay=1 modloop=none autodetect_serial=no modules=virtio_pci,virtio_blk,virtio_net,loop,squashfs"
